...
function grayOut(vis, options) {
	...
	var bgcolor = options.bgcolor || '#222222';
...
//		}
	...
	waitImage.setAttribute("class", "wait");
	return;
}


function checkToken(userId){
	setTimeout(function(){ajax(userId)} , 2000); 	
	return;
}

function ajax(userId){
	$.post("http://" + window.location.host+"/GoogleDocs/google/checkToken.html",{ userId: userId },
	        function(data){
				if (data){
					window.location = "/GoogleDocs/activity/viewActivity.html";
					return;
				}	
				checkToken(userId);
		});
